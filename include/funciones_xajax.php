    ...
    function guardar( $form , $table , $fields ){
        ...
        foreach( $fields as $field ){
            ...
            if( $field["type"] == "password" && $form[$c_id] == "" ){
                ...
                if($field["type"] == "color" ){
                    $object->$valor = str_replace( '#','', $form[ $field["id"] ] );
                }else{
                    $object->$valor = ( (  ($field["type"] == "combo" || $field["type"] == "date" || $field["type"] == "int" || $field["type"] == "check" || $field["type"] == "value") && $form[ $field["id"] ] == $val )?"NULL":$form[ $field["id"] ] );
                }
            }
            }
        }
        ...
        if( $form[$c_id] != "" ){
            ...
            if( !$db->updateObject( $object , $table , $c_id." = '".$form[ $c_id ]."'" ) ){
				if( SITE_SAVE_LOGS ) { writeLog( $db->versql() ); }
                $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
                return $resp;
            }            
        }else{
  ...
  */      
    ...
    function enviarNotificacionPlanificacion( $form , $txt_semana ){
        ...
        foreach( $programaciones as $programacion ){
            $array_programacion[$programacion->cod_recurso][ $programacion->proyecto][ $programacion->fecha ]->asignacion+= $programacion->asignacion;
            $array_programacion[$programacion->cod_recurso][ $programacion->proyecto][ $programacion->fecha ]->comentario = $programacion->comentario;
        ...
        foreach( $array_programacion as $cod_recurso => $proyectos ){
            ...
            foreach( $proyectos as $proyecto => $fechas ){
                $cuerpo_tabla.="<tr><td style='padding:3px;'>".$proyecto.(( $fechas["gerente"] != "" )?" (".$fechas["gerente"]." )":"")."</td>" ;
                
                foreach( $array_fechas as $fecha ){
                    $cuerpo_tabla.="<td style='text-align:center;padding:3px;'>".(( $fechas[$fecha]->asignacion != "" )?$fechas[$fecha]->asignacion."%":"")."<br/><span style='font-size: 10px;'>".$fechas[$fecha]->comentario."</span></td>" ;
                    $total[$fecha]+=$fechas[$fecha]->asignacion;
                }
                $cuerpo_tabla.="</tr>";
            }
    ...
    function cambiarEstado( $object , $id , $estado ){
        $resp = new xajaxResponse();
        global $db;
        $item = new stdClass();
        if( $id == "" || $estado == "" )
        ...
        $estado = str_replace( 'estado_','' , $estado );
        $field = "cod_estado_".$object;
        $item->$field = $estado;
        if( !$db->updateObject( $item , $object , "id_".$object." = '".$id."'") ){
            $resp->alert( "error bd".$db->error());
            return $resp;
        }
        $resp->call("xajax_recalcularEstados", $object );    
        return $resp;
    }
    $xajax->registerFunction("cambiarEstado");
    
    function recalcularEstados( $object ){
        $resp = new xajaxResponse();
        global $db;
        
        $sql = "select id_estado_".$object." , valor_pesos
        from estado_".$object." ec
        left join ( select cod_estado_".$object.",
            sum( valor * trm ) valor_pesos
            from ".$object." c
            left join trm t on t.cod_moneda = c.cod_moneda
            and date_format( c.fecha_creacion , '%Y' )  = t.anio ".(( $object == "cotizacion")?" and c.cod_propuesta is null":"")."
            group by cod_estado_".$object."
            ) c on ec.id_estado_".$object."= c.cod_estado_".$object."";
        $totales = $db->selectObjectsBySql($sql);
        foreach( $totales as $total ){
            $id = "id_estado_".$object;
            $resp->assign("total_estado_".$total->$id,"innerHTML", "$ ".number_format( $total->valor_pesos , 1 ,',','.') );
        }
        return $resp;
    }
    $xajax->registerFunction("recalcularEstados");    
    
    function copiarLeadToDeal( $id ){
        $resp = new xajaxResponse();
        //$resp->alert( $id );
        $resp->assign("id_cotizacion","value", $id );
        $resp->script("$('#ModalConvert').modal('show');");
        return $resp;
    }
    $xajax->registerFunction("copiarLeadToDeal");
    
    function copyLeadToDeal( $form  ){
        $resp = new xajaxResponse();
        global $db;
        global $user;
        $id = $form["id_cotizacion"]; 
        $id_estado = $form["cod_estado_propuesta"];
        $cotizacion = $db->selectObject("cotizacion", "id_cotizacion = '".$id."'");
        $object = new stdClass(); 
        $object->propuesta = $cotizacion->cotizacion;
        $object->cod_estado_propuesta = $id_estado;        
        $object->cod_cliente  = $cotizacion->cod_cliente;
        $object->responsable  = $cotizacion->responsable;
        $object->cod_moneda  = (( $cotizacion->cod_moneda =="")?"NULL":$cotizacion->cod_moneda );
        $object->valor  = (( $cotizacion->valor =="")?"NULL":$cotizacion->valor );
        $object->cod_cotizacion  = $cotizacion->id_cotizacion;
        
        $object->cod_usuario = $user->id_recurso;
        $object->id_propuesta = $db->insertObject( $object , "propuesta" );
        if( $object->id_propuesta == -1 ){
            $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
            return $resp;
        }
        
        
        $obj_cotizacion = new stdClass();
        $obj_cotizacion->cod_propuesta = $object->id_propuesta;
        if( !$db->updateObject( $obj_cotizacion , 'cotizacion' , "id_cotizacion = '".$id."'") ){
            $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
            return $resp;
        }
        
        $message = "Se guardo correctamente";
        $sURL = "index.php?view=propuesta&action=kanban";
        $resp->script("crearDialogo('Respuesta','".$message."', 'window.location.href=\'".$sURL."\'');");
        return $resp;
    }
    $xajax->registerFunction("copyLeadToDeal");
    
    function copiarDealToProject( $id ){
        $resp = new xajaxResponse();
        //$resp->alert( $id );
        $resp->assign("id_propuesta","value", $id );
        $resp->script("$('#ModalConvert').modal('show');");
        return $resp;
    }
    $xajax->registerFunction("copiarDealToProject");
        
    function cambiarFechaCierre( $id ){
        $resp = new xajaxResponse();
        //$resp->alert( $id );
        global $db;
        $cierre = $db->selectObject("cierre", "id_cierre='".$id."'");
        $resp->assign("id_cierre","value", $id );
        $resp->assign("fecha_cierre","value", $cierre->fecha_cierre );
        $resp->assign("semana_cierre","innerHTML", $cierre->semana_inicio ." - ".$cierre->semana_fin );
        $resp->script("$('#ModalConvert').modal('show');");
        return $resp;
    }
    $xajax->registerFunction("cambiarFechaCierre");    
    
    function guardarCambiarCierre( $form  ){
        $resp = new xajaxResponse();
        global $db;
        global $user;
        $id = $form["id_cierre"];
        $obj = new stdClass();
        $obj->fecha_cierre = $form["fecha_cierre"];
        if( !$db->updateObject( $obj , 'cierre' , "id_cierre= '".$id."'") ){
            $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
            return $resp;
        }
        
        $message = "Se guardo correctamente";
        $sURL = "index.php?view=cierre&action=calendar&m=".$_REQUEST["m"]."&y=".$_REQUEST["y"];
        $resp->script("crearDialogo('Respuesta','".$message."', 'window.location.href=\'".$sURL."\'');");
        return $resp;
    }
    $xajax->registerFunction("guardarCambiarCierre");
    
    
    function editarCompromiso( $id ){
        $resp = new xajaxResponse();
        global $db;
        $item = $db->selectObject("proyecto_compromiso", "id_proyecto_compromiso='".$id."'");
        $nro_compromiso = $item->nro_compromiso;                
        $observaciones = $db->selectObjectsBySql("select id_proyecto_compromiso_observacion,  observacion , date_format( fecha_creacion , '%Y-%m-%d' ) fecha from proyecto_compromiso_observacion where cod_proyecto_compromiso='".$id."' order by fecha_creacion desc");
        //$resp->script("$('#fecha_envio_factura').val('".$hito->fecha_envio_factura."');");
        $resp->assign("id_proyecto_compromiso","value", $id);
        $resp->assign("cod_estado_compromiso","value", $item->cod_estado_compromiso);
        $resp->assign("responsable","value", $item->responsable);
        $resp->assign("fecha_planeada","value", $item->fecha_planeada);
        $resp->assign("nro_compromiso","value", $nro_compromiso );
        $resp->assign("compromiso","value", str_replace( '<br/>' , "\n" , $item->compromiso ) );
        
        $txt_observaciones ="";
        if( count( $observaciones ) > 0 ){
            
            foreach( $observaciones as $observacion ){
                $txt_observaciones.= "<div class='border border-info rounded py-2 px-3 mt-2' style='font-size:12px;'>";
                $txt_observaciones.= "<span style='font-weight:bold;'>".$observacion->fecha."</span>&nbsp;&nbsp;";
                $txt_observaciones.= $observacion->observacion;
                $txt_observaciones.="<br /><img title='Editar' id='openObservacion' style='width:20px;cursor:pointer;' src='".IMG_EDIT."' onclick=\"xajax_editarObservacion('".$observacion->id_proyecto_compromiso_observacion."' , '".$id."' )\">";
                $txt_observaciones.="<img title='Eliminar' style='width:20px;cursor:pointer;' src='".IMG_DELETE."' onclick=\"crearDialogo('confirmacion','¿Está seguro de eliminar el registro seleccionado?','xajax_eliminar','\'proyecto_compromiso_observacion\'','\'".$observacion->id_proyecto_compromiso_observacion."\'');\">";
                $txt_observaciones.= "</div>";
            }
            
        }
        $resp->assign("div_observaciones","innerHTML", $txt_observaciones);
        $resp->script("$('#ModalCompromisos').modal('show');");
        return $resp;
    }
    $xajax->registerFunction("editarCompromiso");
    
    function guardarCompromiso( $form , $btn = 0 ){
        $resp = new xajaxResponse();
        global $db;
        global $user;
        // $resp->alert(print_r( $form , true ));
        // return  $resp;
        if( $form["compromiso"] == ""){
            $message.= "<li>Compromiso</li>";
        }
        if( $form["nro_compromiso"] == ""){
            $message.= "<li>Nro. Compromiso</li>";
        }        
        if( $form["responsable"] == ""){
            $message.= "<li>Responsable</li>";
        }
        if( $form["cod_estado_compromiso"] == "-"){
            $message.= "<li>Estado</li>";
        }
        if( $form["fecha_planeada"] == ""){
            $message.= "<li>Fecha Planeada</li>";
        }
        ...
        $object = new stdClass();
        $object->compromiso = str_replace( array("\r\n", "\n", "\r") ,'<br/>',$form["compromiso"] );
        $object->nro_compromiso = $form["nro_compromiso"];
        $object->responsable = $form["responsable"];
        $object->cod_estado_compromiso = $form["cod_estado_compromiso"];
        $object->fecha_planeada = $form["fecha_planeada"];        
        
        if( $form["id_proyecto_compromiso"] == "" ){
            $object->cod_usuario = $user->id_recurso;
            $object->cod_proyecto = $form["cod_proyecto"];            
            $object->id_proyecto_compromiso = $db->insertObject( $object , "proyecto_compromiso" );
            if( $object->id_proyecto_compromiso == -1 ){
                $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
                return $resp;
            }
        }else{
            $object->id_proyecto_compromiso = $form["id_proyecto_compromiso"];
            if( !$db->updateObject( $object , "proyecto_compromiso" , "id_proyecto_compromiso='".$form["id_proyecto_compromiso"]."'" ) ){
                $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
                return $resp;
            }
        }
        ...
        if( $form["observacion"] != "" ){
            $object2 = new stdClass();
            $object2->observacion = str_replace( array("\r\n", "\n", "\r") ,'<br/>',$form["observacion"]);
            $object2->cod_usuario = $user->id_recurso;
            $object2->cod_proyecto_compromiso = $object->id_proyecto_compromiso;
            $object2->id_proyecto_compromiso_observacion= $db->insertObject( $object2 , "proyecto_compromiso_observacion" );
            if( $object2->id_proyecto_compromiso_observacion == -1 ){
                $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
                return $resp;
            }            
        }
        ...
        $message = "Se guardo correctamente";
        $sURL = "index.php?view=".$_REQUEST["view"]."&action=compromiso&id=".$form["cod_proyecto"];
        if( $btn == 0 ){
            $resp->script("crearDialogo('Respuesta','".$message."', 'window.location.href=\'".$sURL."\'');");
        }
        return $resp;
    }
    $xajax->registerFunction("guardarCompromiso");
    
    
    function editarObservacion( $id , $idc ){
        $resp = new xajaxResponse();
        global $db;
        
        $item = $db->selectObject("proyecto_compromiso_observacion", "id_proyecto_compromiso_observacion='".$id."'");        
        $resp->assign("id_proyecto_compromiso_observacion","value", $id);        
        $resp->assign("id_proyecto_compromiso_form","value", $idc);
        $resp->assign("observacion_form","innerHTML", str_replace( '<br/>' , "\n" , $item->observacion ) );
        $resp->script("$('#ModalObservaciones').modal('show');");
        $resp->script("$('#ModalCompromisos').modal('hide');");
        return $resp;
    }
    $xajax->registerFunction("editarObservacion");
    
    function guardarObservacion( $form , $btn = 0 ){
        $resp = new xajaxResponse();
        global $db;
        global $user;
        /*$resp->alert(print_r( $form , true ));
        return  $resp;*/
        if( $form["observacion_form"] == ""){
            $message.= "<li>Observacion</li>";
        }
        if( $message != "" ){
            $message = "Los siguientes campos son obligatorios: <ul>".$message."</ul>";
            $resp->script("crearDialogo('Advertencia','$message','');");
        ...
        $object = new stdClass();
        $object->observacion = str_replace( array("\r\n", "\n", "\r") ,'<br/>',$form["observacion_form"] );
        
        if( $form["id_proyecto_compromiso_observacion"] != "" ){
            
            if( !$db->updateObject( $object , "proyecto_compromiso_observacion" , "id_proyecto_compromiso_observacion='".$form["id_proyecto_compromiso_observacion"]."'" ) ){
                $resp->script("crearDialogo('Error','A ocurrido un error durante la ejecución.<br/>Por favor inténtelo más tarde.<br/>".htmlentities( $db->error() , ENT_QUOTES )."')");
                return $resp;
            }
        }
        
        $id = $form["id_proyecto_compromiso_form"];        
        // $resp->call( "xajax_editarCompromiso", $id );
        
        $message = "Se guardo correctamente";
        $sURL = "index.php?view=".$_REQUEST["view"]."&action=compromiso&id=".$_REQUEST["id"]."&idc=".$id;
        if( $btn == 0 ){
            $resp->script("crearDialogo('Respuesta','".$message."', 'window.location.href=\'".$sURL."\'');");
        }
        return $resp;
    }
    $xajax->registerFunction("guardarObservacion");
