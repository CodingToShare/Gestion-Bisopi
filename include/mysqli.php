...
class mysqli_database {
	...
	var $havedb = false;
	var $prefix = "";
	var $validos = array( "NULL", "null", "CURRENT_DATE", "CURRENT_TIMESTAMP");
	var $ssqqll = "";
	/* exdoc
	...
	function connect($username,$password,$hostname,$database,$new=false) {
		...
		if (version_compare(phpversion(),'4.2.0','>=') > 0) {
			...
			if(!$this->connection = @mysqli_connect($hostname,$username,$password,$new)) {
		...
		if ($this->connection) {
            ...
            if (!$this->havedb = (@mysqli_select_db($this->connection,$database) ? true : false)) {
	...
	function toggle($table, $col, $where=null) {
                ...
                return $this->updateObject($obj, $table, $where);
        }
	...
	function selectObjects($table, $where = null,$orderby = null) {
		...
		$this->ssqqll = "SELECT * FROM `" . $this->prefix . "$table` WHERE $where $orderby";
	...
	function selectArraysBySql($sql) {
	    ...
	    if ($res == null) return array();
	    $objects = array();
	    for ($i = 0; $i < mysqli_num_rows($res); $i++) $objects[] = mysqli_fetch_array($res , MYSQLI_ASSOC );
	    return $objects;
	}
	...
	function selectSum($table,$col,$where = null) {
                if ($where == null) $where = "1";
                $sql = "SELECT SUM(".$col.") FROM `" . $this->prefix . "$table` WHERE $where";
                $this->ssqqll = $sql;
                $res = @mysqli_query($this->connection, $sql );
	...
	*/
	...
	function selectObject($table,$where) {
		...
		$this->ssqqll = "SELECT * FROM `" . $this->prefix . "$table` WHERE $where";
			if ($res == null) return null;
		...
		foreach (get_object_vars($object) as $var=>$val) {
			...
			if ($var{0} != '_') {
				...
				if (version_compare(phpversion(),'4.0.3','>=') > 0) {
					if( array_search( $val , $this->validos ) === false )
	...
	function delete($table,$where = null) {
		if ($where != null) {
			$this->ssqqll = "DELETE FROM `" . $this->prefix . "$table` WHERE $where";
		...
		foreach (get_object_vars($object) as $var=>$val) {
			...
			if ($var{0} != '_') {
				if (version_compare(phpversion(),'4.0.3','>=') > 0) {
					...
					if( array_search( $val , $this->validos ) === false )
				   		$sql .= "`$var`='".mysqli_escape_string($this->connection,$val)."',";
				   	else
				   		$sql .= "`$var`=".mysqli_escape_string($this->connection,$val).",";
			   }
			}
	...
	function error() {
		if ($this->connection && mysqli_errno($this->connection) != 0) {
			...
			switch ($errno) {
				...
				case 1062:
					return "1062 : El registro ya se encuentra en la base de datos.";
				default:
...
$db->connect(DB_USERNAME,DB_PASSWORD,DB_SERVER,DB_NAME);
?>
